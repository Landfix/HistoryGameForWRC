...
public abstract class SerializableDictionaryBase<TKey, TValue, TValueStorage> : SerializableDictionaryBase, IDictionary<TKey, TValue>, IDictionary, ISerializationCallbackReceiver, IDeserializationCallback, ISerializable
{
    ...
    public int RemoveAll(Func<KeyValuePair<TKey, TValue>, bool> functor)
    {
        if (functor == null)
            return 0;

        var enumerator = ((IDictionary<TKey, TValue>) m_dict).GetEnumerator();
        var keysList = new List<TKey>();
        var count = 0;

        while (enumerator.MoveNext())
        {
            if (functor(enumerator.Current))
            {
                keysList.Add(enumerator.Current.Key);
                count++;
            }
        }
